...
/* puchMsg 要进行CRC校验的消息 ,usDataLen 消息中字节数*/
UINT16 MYAPI CalCRC16(void *puchMsg1, int usDataLen)
{
	BYTE uchCRCHi = 0xFF ; /* 高CRC字节初始化 */
	BYTE uchCRCLo = 0xFF ; /* 低CRC 字节初始化 */
	UINT32 uIndex ; /* CRC循环中的索引 */
	unsigned char *puchMsg = (unsigned char *)puchMsg1 ;
...
UINT32 MYAPI CalCRC32(void *pData1, int blk_len)
{
 ...
 /* auchMsg 要进行计算的消息   usDataLen LRC 要处理的字节的数量*/
BYTE MYAPI LRC(BYTE *auchMsg,UINT16 usDataLen)
{ 
	BYTE uchLRC = 0 ; /* LRC 字节初始化 */
