...
char MYAPI GetBase64ExChar(int index)
{
...
signed char MYAPI GetBase64ExIndex(unsigned char ch)
{
    char  index = (char)-1;
    if (ch >= 'A' && ch <= 'Z')
    {
                        ...
                        char * szB64Out, int nB64BufLen)
{
    ...
    {
        szBase64Unit[0]=GetBase64ExChar((*psrc)>>2);
        Tmp1=*psrc<<6;
        Tmp1=Tmp1>>2;
        Tmp2=*(psrc+1)>>4;

        szBase64Unit[1]=GetBase64ExChar(Tmp1|Tmp2);

        Tmp1=*(psrc+1)<<4;
        Tmp1=Tmp1>>2;
        Tmp2=(*(psrc+2)>>6);

        szBase64Unit[2]=GetBase64ExChar(Tmp1|Tmp2);

        Tmp1=*(psrc+2)<<2;
        Tmp1=Tmp1>>2;
        szBase64Unit[3]=GetBase64ExChar(Tmp1);
    ...
    if(i<nDataLen)
    {
        ...
        {
            Tmp2=0;
            p64[0]=GetBase64ExChar((*psrc)>>2);
            Tmp1=*psrc<<6;Tmp1=Tmp1>>2;
            if(nRest>1)Tmp2=*(psrc+1)>>4;
            p64[1]=GetBase64ExChar(Tmp1|Tmp2);
        }
        if(nRest>1)
        {
            Tmp2=0;
            ...
            p64[2]=GetBase64ExChar(Tmp1|Tmp2);
...
int Base64ExFetch4Code(char **pCode,signed char *Base64Index,char *szStopCharList,char *Code64Ok)
{
    int i;
    ...
    for(i=0;i<4;i++)
    {
        ...
        for( ;(**pCode)!=0&&
         ...
         (Base64Index[i]=GetBase64ExIndex((unsigned char)(**pCode)))<0;(*pCode)++);
...
int Base64ExFetch4CodeNoStopList(char **pCode,signed char *Base64Index,char *Code64Ok)
{
    int i;
    for(i=0;i<4;i++){Base64Index[i]=(char)-1;Code64Ok[i]=0;}
    ...
    for(i=0;i<4;i++)
    {
        ...
        for( ;(**pCode)!=0 && ((Base64Index[i])=GetBase64ExIndex(**pCode)) < 0;(*pCode)++);
...
int MYAPI Base64ExDecode(const char * szCode, void * vDataOut, int nDataBufLen)
{
    ...
    while((nRest=Base64ExFetch4CodeNoStopList((char **)&p64Code,Base64Index,pCode))==4)
    {
        Tmp0=Base64Index[0];
        Tmp1=Base64Index[1];
        Tmp0=Tmp0<<2;
                        ...
                        void * vDataOut, int nDataBufLen)
{
    ...
    while((nRest=Base64ExFetch4Code((char **)&p64Code,Base64Index,szStopCharList,pCode))==4)
    {
        Tmp0=Base64Index[0];
        Tmp1=Base64Index[1];
        Tmp0=Tmp0<<2;
