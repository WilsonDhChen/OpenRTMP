                                        ...
                                        size_t val_len )
{
    ...
    if( ( cur = asn1_find_named_data( *head, oid, oid_len ) ) == NULL )
    {
        ...
        //
        if( ( cur = (asn1_named_data*)polarssl_malloc( sizeof(asn1_named_data) ) ) == NULL )
            return( NULL );

        memset( cur, 0, sizeof(asn1_named_data) );

        cur->oid.len = oid_len;
        cur->oid.p = (unsigned char*)polarssl_malloc( oid_len );
        if( cur->oid.p == NULL )
        {
            polarssl_free( cur );
            return( NULL );
        ...
        cur->val.len = val_len;
        cur->val.p = (unsigned char*)polarssl_malloc( val_len );
        if( cur->val.p == NULL )
        {
