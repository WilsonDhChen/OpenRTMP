...
public:
    ...
    static BOOL IsSupportSsl();
       
    BOOL SetSslCertAndKey(const char *szCertFile = NULL, const char *szKeyFile = NULL);
    void SetSslType(NetSsl ssl = NetSslNone);
    
    void SetSessionTimeout(int nSeconds = -1);
    void SetSessionTimeoutType(SessionTimeoutType type);
    SessionTimeoutType GetSessionTimeoutType();

    void SetMaxConnections(int nCount = 2000);
    int  GetMaxConnections();

    void SetMaxSends(int nCount = 100);
    void SetMaxWrittingBytes(int bytes);
